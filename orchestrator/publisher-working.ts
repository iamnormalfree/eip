...
import { submitComplianceValidationJob } from '../lib_supabase/queue/eip-queue-with-compliance';
...
interface PublishResult {
  ...
  compliance_job_submitted: boolean; // Whether compliance job was submitted
  compliance_job_id: string; // ID of submitted compliance validation job
}
...
export async function publishArtifact(input: PublishInput): Promise<PublishResult> {
  ...
  // ============================================================================
  // AUTOMATIC COMPLIANCE VALIDATION JOB SUBMISSION
  // ============================================================================
  let complianceJobSubmitted = false;
  let complianceJobId = '';

  try {
    // Extract sources for compliance validation
    const sources = input.retrieval.candidates?.map(candidate => candidate.url) || [];

    // Submit compliance validation job if there are sources or regulatory indicators
    if (sources.length > 0 || input.draft.toLowerCase().includes('singapore') ||
        input.draft.toLowerCase().includes('financial') || input.draft.toLowerCase().includes('investment')) {

      const complianceJobResult = await submitComplianceValidationJob({
        job_id: `publish-${Date.now()}`,
        content: input.draft,
        context: {
          title: frontmatter.title || 'Untitled Content',
          target_audience: frontmatter.target_audience || 'general',
          content_type: contentType,
          geographical_focus: frontmatter.geographical_focus || 'singapore',
          language: frontmatter.language || 'en'
        },
        sources: sources,
        artifact_id: `artifact-${Date.now()}`,
        correlation_id: input.metadata?.correlation_id,
        priority: humanReviewRequired ? 1 : 5, // Higher priority for content requiring review
        validation_level: humanReviewRequired ? 'comprehensive' : 'standard'
      });

      if (complianceJobResult.success) {
        complianceJobSubmitted = true;
        complianceJobId = complianceJobResult.jobId;
        console.log('✅ Publisher: Compliance validation job submitted:', complianceJobId);
      } else {
        console.warn('⚠️ Publisher: Failed to submit compliance job:', complianceJobResult.error);
      }
    } else {
      console.log('ℹ️ Publisher: No compliance validation required (no sources or regulatory indicators)');
    }
  } catch (complianceError) {
    console.error('❌ Publisher: Error submitting compliance job:', complianceError);
  ...
  return {
    ...
    frontmatter,
    metrics,
    compliance_job_submitted: complianceJobSubmitted,
