...
class EventBusImpl {
  ...
  private constructor() {
    ...
    try {
      ...
      // Gracefully handle window attachment errors during construction
      console.warn("Failed to attach event bus during construction:", error)
    }
  }

  /**
   * Attach event bus to window with dual-attachment strategy
   * Implements event_bus_strategy::dual_attachment_canonical_eip
   */
  /**
   * Attach event bus to window with dual-attachment strategy
   * Implements event_bus_strategy::dual_attachment_canonical_eip
  ...
  private attachToWindow(): void {
    try {
      // Always set canonical EIP reference
      (window as any).__eip_eventbus = this
      
      // Dual-attachment in compatibility mode
      if (isLegacyCompat()) {
        (window as any).__nextnest_eventbus = this
      }
    } catch (error) {
      console.warn("Failed to attach event bus to window:", error)
      // Don't re-throw - the event bus should still work even if window attachment fails
    }
  }
  ...
  reset(): void {
    ...
    // Clean up window properties before re-attaching
    if (typeof window !== 'undefined') {
      delete (window as any).__eip_eventbus
      delete (window as any).__nextnest_eventbus
      
      // Re-attach to window after reset if in browser environment
      try {
        this.attachToWindow()
      } catch (error) {
        console.warn("Failed to re-attach event bus during reset:", error)
      }
    }
  }
