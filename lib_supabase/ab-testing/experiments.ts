...
export const CTA_COPY_VARIANTS = {
  ...
  social_proof: 'Continue (10,000+ helped this month)'
} as const
...
/**
 * Gets session ID for experiment bucketing using dual-read pattern
 */
function getSessionId(): string {
  // Early return for SSR environment
  if (typeof globalThis?.window === 'undefined') {
    return 'ssr_session'
  }

  try {
    // Try to get existing session ID using dual-read pattern
    let sessionId = compat.getWithAliases(['eip_session_id', 'nextnest_session_id'], 'sessionStorage')

    if (!sessionId) {
      // Generate new session ID with higher precision timestamp
      const timestamp = Date.now()
      const random1 = Math.random().toString(36).substring(2, 11)
      const random2 = Math.random().toString(36).substring(2, 7)
      sessionId = `session_${timestamp}_${random1}_${random2}`
      // Store using single-write pattern
      compat.setCanonical('eip_session_id', sessionId, 'sessionStorage')
    }

    return sessionId || 'fallback_session'
  } catch (error) {
    // Graceful error handling for storage access issues
    console.warn('Failed to get/set session ID:', error)
    const timestamp = Date.now()
    const random = Math.random().toString(36).substring(2, 9)
    return `fallback_session_${timestamp}_${random}`
  }
}
...
export function getExperimentVariant(experimentId: string): string {
  ...
  try {
    const forcedVariant = compat.getWithAliases([`force_${experimentId}`], 'sessionStorage')
    if (forcedVariant && experiment.variants.includes(forcedVariant)) {
      return forcedVariant
    }
  } catch (error) {
    // Graceful error handling for forced variant lookup
    console.warn(`Failed to get forced variant for ${experimentId}:`, error)
  }
...
export function getActiveExperiments(): Record<string, string> {
  ...
  try {
    ...
    console.warn('Failed to get active experiments:', error)
    return {}
  }
}
