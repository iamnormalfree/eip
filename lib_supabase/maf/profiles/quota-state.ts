...
export class QuotaStateManager {
  ...
  private calculateRollingWindows(state: QuotaState, now: number): RollingWindow[] {
    ...
    const windowDuration = this.configuration.rollingWindowHours; // 5 hours
    ...
    // Create rolling windows covering the last 24 hours
    const endTime = now;
    const startTime = endTime - (24 * 60 * 60 * 1000); // 24 hours ago

    for (let windowStart = startTime; windowStart < endTime; windowStart += (windowDuration * 60 * 60 * 1000)) {
      const windowEnd = windowStart + (windowDuration * 60 * 60 * 1000);
