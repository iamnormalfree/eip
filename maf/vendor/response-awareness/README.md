...
## Semantic Skill Catalog

**Token-efficient resource discovery through ChromaDB vector search**

The skill catalog provides Claude with contextually relevant resources without burning tokens on massive upfront context. Skills are discovered semantically as you work.

**Features:**
- **On-Demand Discovery**: Relevant skills surface when reading related files
- **Proactive Suggestions**: Skills suggested based on your prompt before work begins
- **Skill Gap Detection**: Identifies areas where no relevant skills exist
- **Session Tracking**: Each skill shown once per session (re-notifies on higher relevance)
- **Multi-Skill Display**: Up to 3 relevant skills per notification
...
# List all skills
python .claude/hooks/setup_chromadb_example.py list
...
User sends prompt
         ↓
UserPromptSubmit hook searches for relevant skills
         ↓
[SKILL SUGGESTIONS] shown if matches found (>30% relevance)
         ↓
Claude reads files during work
         ↓
PostToolUse hook finds related skills
         ↓
[SKILL CATALOG] shown with up to 3 matches (>25% relevance)
         ↓
If no matches: [SKILL GAP] recorded for later review
         ↓
Session ends → Tracking resets
```

### Skill File Format

Create a markdown file with your skill content:
...
The first paragraph becomes the description shown in suggestions.
...
- **`plan-synthesis-agent`** (Sonnet 4.5 + extended_thinking): Maximum observer capacity for cross-domain conflict resolution and weight resistance. Holds all PATH_DECISION points simultaneously to evaluate system-wide coherence. Creates unified blueprints by choosing paths based on integration quality rather than individual statistical likelihood.

- **`metacognitive-tag-verifier`** (Sonnet 4.5): Specialized training for tag taxonomy recognition and resolution. Fresh context prevents correction resistance when validating implementation assumptions. Systematically resolves observer-marked recognition events and removes pattern-driven code.
