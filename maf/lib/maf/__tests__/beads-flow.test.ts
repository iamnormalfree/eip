...
import { createBeadsScheduler, type MafScheduler, type MafTaskSummary } from '../scheduling/scheduler';
import { createFileBasedRuntimeState, type MafRuntimeState } from '../core/runtime-state';
import { createMafCoordinator, type MafCoordinator } from '../core/coordinator';
import { beadsReady, beadsAssign, beadsInit, beadsCreate, type BeadsTask } from '../beads/cli';
  ...
  describe('File-Based Runtime State Integration', () => {
    it('should persist file reservations to .agent-mail directory', async () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain('already leased');
      }
    }, TEST_TIMEOUT);
