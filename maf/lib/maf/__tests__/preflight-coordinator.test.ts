...
jest.mock('../evidence/collector', () => ({
  EvidenceCollector: jest.fn().mockImplementation(() => ({
    collectEvidence: jest.fn<() => Promise<{ metadata: { executionId: string } }>>().mockResolvedValue({
      metadata: { executionId: 'test-execution-id' }
    }),
    storeExecutionResult: jest.fn<() => Promise<void>>().mockResolvedValue(undefined)
  }))
}));

jest.mock('../state/agent', () => ({
  AgentStateManager: jest.fn().mockImplementation(() => ({
    getAgentState: jest.fn<() => Promise<{ id: string; status: string }>>().mockResolvedValue({ id: 'test-agent', status: 'active' }),
    updateAgentState: jest.fn<() => Promise<void>>().mockResolvedValue(undefined)
  }))
}));
...
jest.mock('../core/runtime-factory', () => ({
  createMafRuntimeStateFromEnv: (jest.fn() as any).mockResolvedValue({
    enqueue: (jest.fn() as any).mockResolvedValue(undefined),
    upsertHeartbeat: (jest.fn() as any).mockResolvedValue(undefined)
  } as any)
}));
...
describe('MafPreflightCoordinator', () => {
  ...
  describe('Python Validation', () => {
    ...
    it('should not rely on shell execution for python or pip checks', async () => {
      const childProcess = require('child_process');
      const mockExecFileSync = jest.spyOn(childProcess, 'execFileSync');
      mockExecFileSync.mockImplementation(((command: string, args?: string[]): string => {
        if (command === 'python3' && args?.includes('--version')) {
  ...
  describe('Complete Preflight Check', () => {
    it('should pass all validations and return success result', async () => {
      ...
      expect(result.validations.overall).toBe('passed');
    ...
    it('should fail with comprehensive error details when validations fail', async () => {
      ...
      // Set up incomplete environment - remove all required env vars to ensure failure
      delete process.env.OPENAI_API_KEY;
      delete process.env.ANTHROPIC_API_KEY;
      delete process.env.MAF_AGENT_ID;
      ...
      expect(result.validations.overall).toBe('failed');
      expect(result.validations.errors.length).toBeGreaterThan(0);
    ...
    it('should output machine-readable JSON format', async () => {
      const childProcess = require('child_process');
      const mockExecFileSync = jest.spyOn(childProcess, 'execFileSync');
      mockExecFileSync.mockReturnValue('Python 3.11.0\n');
...
describe('CAN-063 Compliance Tests', () => {
  describe('Default Behavior Requirements', () => {
    it('should default to full validation mode for CAN-063 compliance', async () => {
      ...
      const mockProcessExit = jest.spyOn(process, 'exit').mockImplementation(() => { throw new Error('process.exit called'); });
      ...
      // Verify that full validation was performed and exit was called with failure code
    ...
    it('should exit non-zero when prerequisites are missing in default mode', async () => {
      // Set up test environment with no prerequisites
      const testDir = '/tmp/maf-test-' + Math.random().toString(36).substring(7);
      process.env.MAF_AGENT_MAIL_ROOT = testDir;
