...
export BEADS_PROJECT_PATH="${PROJECT_ROOT}"

# Test 1: Unified Memory Service Status
echo ""
echo "1. Testing Unified Memory Service Status..."
python3 ${PROJECT_ROOT}/scripts/maf/memory-service-unified.py status
...
echo "Test content: Created new feature file, decided to use TypeScript, fixed import error" | \
    python3 ${PROJECT_ROOT}/scripts/maf/memory-service-unified.py store \
        --agent "test-agent" \
        --bead "bd-123" \
        --file -
...
python3 ${PROJECT_ROOT}/scripts/maf/memory-service-unified.py retrieve \
    --agent "test-agent" \
    --bead "bd-123" \
...
echo "Test decision: Will implement authentication middleware" | \
    ${PROJECT_ROOT}/scripts/maf/agent-memory.sh store
...
${PROJECT_ROOT}/scripts/maf/agent-memory.sh restore > /tmp/test-restore.txt
...
${PROJECT_ROOT}/scripts/maf/context-manager-v2.sh status
