...
# NOTE: PROJECT_ROOT should be set by the parent script (spawn-agents.sh or prompt-agent.sh)
# using auto-detection that handles both subtree and direct layouts.
# We only set a fallback here if not already set.

SCRIPT_DIR="${SCRIPT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"

# Use PROJECT_ROOT from parent if available, otherwise try to detect
# This is a fallback - the parent script should always set PROJECT_ROOT correctly
if [[ -z "${PROJECT_ROOT:-}" ]]; then
    # No parent set PROJECT_ROOT - try to detect from lib directory
    # Check if we are in subtree (maf/scripts/maf/lib) or direct (scripts/maf/lib) layout
    if [[ "$SCRIPT_DIR" == *"/maf/scripts/maf/lib" ]]; then
        # Subtree: go up 4 levels from lib to project root
        PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
    elif [[ "$SCRIPT_DIR" == *"/scripts/maf/lib" ]]; then
        # Direct: go up 3 levels from lib to project root
        PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
    else
        # Unknown layout - search for package.json
        PROJECT_ROOT="$(pwd)"
