...
get_agent_name_from_topology() {
    local pane_index="$1"
    local topology_file="${MAF_TOPOLOGY_FILE:-$PROJECT_ROOT/.maf/config/agent-topology.json}"
...
get_restart_command_from_topology() {
    ...
    case "$pane_index" in
        ...
        2|3) echo "${MAF_IMPLEMENTOR_CMD:-claude --settings $PROJECT_ROOT/.claude}" ;;
        *) echo "${MAF_DEFAULT_AGENT_CMD:-claude --settings $PROJECT_ROOT/.claude}" ;;
    esac
    return 0
}
