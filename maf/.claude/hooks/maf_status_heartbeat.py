...
class MAFStatusHeartbeat:
    ...
    def _detect_role(self) -> str:
        ...
        # Auto-detect project root by finding .maf/config/agent-topology.json
        ...
        if pane_index and pane_index.isdigit():
            ...
            # Look for .maf/config/agent-topology.json by traversing up
            search_path = cwd
            for _ in range(5):  # Max 5 levels up
                candidate = os.path.join(search_path, '.maf', 'config', 'agent-topology.json')
    ...
    def _detect_bead(self) -> str:
        ...
        - "Implement bead [project-prefix]-[id]"
        - "bd start [project-prefix]-[id]"
        - "Bead [id]:"
        """
        ...
        # Pattern 4: "bead [project-prefix]-" in recent context
