...
send_review_rejection() {
  ...
  # Check if should give up (3 attempts)
...
# Verify TDD and notify completion (with TDD gatekeeping)
verify_and_notify_complete() {
  local bead_id="$1"
  local agent_name="$2"
  local agent_pane="$3"

  # Verify TDD workflow before notifying reviewer
  if ! verify_tdd_workflow "$bead_id"; then
    warn "TDD verification failed for $bead_id, not notifying reviewer"
    ...
    send_agent_mail \
      "$(get_mail_name_by_role "supervisor")" \
      "$(get_mail_name_by_pane "$agent_pane")" \
      "[$bead_id] TDD Violation Detected" \
  ...
  # TDD verified - notify reviewer
  notify_bead_complete "$bead_id" "$agent_name"
  return 0
}
...
get_ready_ids() {
  ...
  # Prioritize using BV (Bead Vision)
  local prioritized
  prioritized=$(python3 maf/scripts/maf/lib/bv_integration.py --prioritize $ready_beads 2>/dev/null || echo "$ready_beads")

  # Extract just the bead IDs from BV output (format: "  1. bead-id")
  echo "$prioritized" | grep -E '^\s+\d+\.' | sed 's/^\s*\d\+\.\s*//' | head -20
}
prompt_agent() {
  local pane="$1"
  local message="$2"
...
assign_beads() {
  ...
  # Send TDD instructions first
  prompt_agent "$target_pane" "echo '=== TDD WORKFLOW INSTRUCTIONS ===' && echo '' && cat <<'TDD_PROMPT_END'
$tdd_prompt
TDD_PROMPT_END
"
  prompt_agent "$target_pane" "echo '=====================================' && echo 'Starting TDD workflow now...' && echo ''"
  ...
  log "Updating BV progress dashboard..."
  python3 maf/scripts/maf/lib/bv_integration.py --progress 2>/dev/null || true
...
autonomous_loop() {
  ...
  python3 maf/scripts/maf/lib/bv_integration.py --triage > /tmp/bv-triage.json 2>/dev/null || true
