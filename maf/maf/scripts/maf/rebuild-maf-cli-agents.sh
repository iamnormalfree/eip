#!/bin/bash
# ABOUTME: Rebuild script for maf-cli:agents tmux session with Claude for all agents
# ABOUTME: Restores 4-pane layout with correct roles: Supervisor(Claude), Reviewer(Claude), Imp-1(Claude), Imp-2(Claude)
# COMPLETION_DRIVE: Implements roundtable-2kt.3 based on synthesis blueprint
# LCL: maf_mode::all_claude
...
if [[ "$DETECTED_SCRIPT_DIR" == *"/maf/scripts/maf" ]]; then
    # Subtree layout: maf/scripts/maf/ -> go up 3 levels (maf/scripts -> maf -> root)
    PROJECT_ROOT="$(cd "$DETECTED_SCRIPT_DIR/../../.." && pwd)"
else
    # Direct layout: scripts/maf/ -> go up 2 levels (scripts -> root)
    PROJECT_ROOT="$(cd "$DETECTED_SCRIPT_DIR/../.." && pwd)"
fi
...
# This loads BEFORE topology config, so .maf/config/agent-topology.json can override
...
# Configuration - Read SESSION_NAME from environment, then topology config
# Priority: 1) .maf-config (MAF_TMUX_SESSION), 2) topology config, 3) default
SESSION_NAME="${MAF_TMUX_SESSION:-maf-cli}"  # Default from .maf-config or maf-cli
if [[ -f "$TOPOLOGY_FILE" ]]; then
    CONFIG_SESSION_NAME=$(jq -r '.session_name // empty' "$TOPOLOGY_FILE" 2>/dev/null)
    if [[ -n "$CONFIG_SESSION_NAME" && "$CONFIG_SESSION_NAME" != "null" ]]; then
...
if tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    ...
    echo ""
    echo "You have a session name conflict. Choose a unique name in .maf/config/agent-topology.json:"
    echo "  \"session_name\": \"maf-your-project\""
    echo ""
...
# COMPLETION_DRIVE: Pane 0 (top) = Supervisor - Claude
# COMPLETION_DRIVE: Pane 1 (bottom) = Reviewer - Claude
# COMPLETION_DRIVE: Pane 2 = Implementor-1 - Claude
# COMPLETION_DRIVE: Pane 3 = Implementor-2 - Claude

# Default commands (can be overridden via environment)
SUPERVISOR_CMD="${MAF_SUPERVISOR_CMD:-claude --settings ${PROJECT_ROOT}/.claude}"
REVIEWER_CMD="${MAF_REVIEWER_CMD:-claude --settings ${PROJECT_ROOT}/.claude}"
...
# Get agent name for pane from topology
get_agent_name() {
    local pane_index="$1"

    if [[ -f "$TOPOLOGY_FILE" ]]; then
        jq -r ".panes[] | select(.index == $pane_index) | .agent_name // empty" "$TOPOLOGY_FILE" 2>/dev/null
    fi
}
        ...
        if [[ -x "$MEMORY_SCRIPT" ]]; then
            ...
            for pane_idx in 0 1 2 3; do
                ...
                if tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
                    ...
                    if [[ -n "$context" ]]; then
                        # Get agent name from topology
                        local agent_name
                        agent_name=$(get_agent_name "$pane_idx")
                        if [[ -z "$agent_name" ]]; then
                            agent_name="agent_${pane_idx}"
                        fi
...
start_agent() {
    ...
    # Check for specialized agent prompt
    local agent_prompt_file="${PROJECT_ROOT}/.maf/agents/$(echo "$agent_name" | tr '[:upper:]' '[:lower:]')-prompt.md"
    local has_specialized_prompt=false

    if [[ -f "$agent_prompt_file" ]]; then
        has_specialized_prompt=true
        log_info "  Loading specialized prompt: $(basename "$agent_prompt_file")"
    fi
    ...
    if [[ -n "$worktree_path" && -d "$worktree_path" ]]; then
        ...
        # If specialized prompt exists, display it first
        if [[ "$has_specialized_prompt" == "true" ]]; then
            display_agent_prompt "$pane" "$agent_prompt_file" "$agent_name"
            sleep 1
        fi

        # Start agent in worktree with identity wrapper
        ...
        # Start agent in project root with identity wrapper
        local wrapped_cmd="bash $AGENT_STARTUP_WRAPPER '$agent_name' '$agent_prompt_file' $agent_cmd"
        tmux send-keys -t "$pane" -l "$wrapped_cmd"
        tmux send-keys -t "$pane" Enter
    fi
...
# Display agent prompt in pane
display_agent_prompt() {
    local pane="$1"
    local prompt_file="$2"
    local agent_name="$3"

    # Display a message about the specialized prompt
    tmux send-keys -t "$pane" -l "echo -e '\033[1;36m═══════════════════════════════════════════════════\033[0m'"
    tmux send-keys -t "$pane" Enter
    tmux send-keys -t "$pane" -l "echo -e '\033[1;36m  Agent: ${agent_name}\033[0m'"
    tmux send-keys -t "$pane" Enter
    tmux send-keys -t "$pane" -l "echo -e '\033[1;36m  Specialized prompt loaded from: $(basename "$prompt_file")\033[0m'"
    tmux send-keys -t "$pane" Enter
    tmux send-keys -t "$pane" -l "echo -e '\033[1;36m═══════════════════════════════════════════════════\033[0m'"
    tmux send-keys -t "$pane" Enter
    tmux send-keys -t "$pane" -l "echo ''"
    tmux send-keys -t "$pane" Enter

    # Store prompt file path in environment for agent to reference
    tmux send-keys -t "$pane" -l "export AGENT_PROMPT_FILE='$prompt_file'"
    tmux send-keys -t "$pane" Enter
    tmux send-keys -t "$pane" -l "export AGENT_NAME='$agent_name'"
    tmux send-keys -t "$pane" Enter
}
...
configure_agents() {
    ...
    # Get agent names from topology
    local agent_0_name
    local agent_1_name
    local agent_2_name
    local agent_3_name

    agent_0_name=$(get_agent_name 0)
    agent_1_name=$(get_agent_name 1)
    agent_2_name=$(get_agent_name 2)
    agent_3_name=$(get_agent_name 3)

    # Use defaults if topology file doesn't exist
    agent_0_name="${agent_0_name:-GreenMountain}"
    agent_1_name="${agent_1_name:-BlackDog}"
    agent_2_name="${agent_2_name:-OrangePond}"
    agent_3_name="${agent_3_name:-FuchsiaCreek}"

    # Pane 0 (top): Supervisor
    start_agent 0 "$agent_0_name" "$SUPERVISOR_CMD"
    restore_agent_state 0 "$agent_0_name"

    # Pane 1 (bottom): Reviewer
    start_agent 1 "$agent_1_name" "$REVIEWER_CMD"
    restore_agent_state 1 "$agent_1_name"

    # Pane 2: Implementor-1
    start_agent 2 "$agent_2_name" "$IMPLEMENTOR_CMD"
    restore_agent_state 2 "$agent_2_name"

    # Pane 3: Implementor-2
    start_agent 3 "$agent_3_name" "$IMPLEMENTOR_CMD"
    restore_agent_state 3 "$agent_3_name"
...
show_summary() {
    ...
    echo "Panes:"
    echo "  0 (top-left):    $agent_0_name (Supervisor) - $SUPERVISOR_CMD"
    echo "  1 (bottom-left): $agent_1_name (Reviewer) - $REVIEWER_CMD"
    echo "  2 (top-right):   $agent_2_name (Implementor-1) - $IMPLEMENTOR_CMD"
    echo "  3 (bottom-right): $agent_3_name (Implementor-2) - $IMPLEMENTOR_CMD"
    echo ""
