...
export async function mirrorSupabaseToNeo4j(
  ...
  const createdDrivers: { neo4j: boolean; supabase: boolean } = {
    neo4j: false,
    supabase: false
  };
  ...
  try {
    ...
    console.log('Starting mirror operation...', { 
      since: since || 'full', 
      graphSparse,
      hasInjectedNeo4j: !!clients?.neo4j,
      hasInjectedSupabase: !!clients?.supabase,
      hasNeo4jDriver: !!neo4jDriver,
    ...
    if (neo4jDriver && createdDrivers.neo4j) {
...
async function processConceptNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  ...
  try {
    ...
    return { created, processed, maxTimestamp };
...
async function processPersonaNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  let maxTimestamp: Date | undefined;
...
async function processOfferNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  let maxTimestamp: Date | undefined;
...
async function processEvidenceNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  let maxTimestamp: Date | undefined;
  ...
  try {
    ...
    for (const ev of evidence) {
      ...
      if (session) {
        const cypher = 'MERGE (e:Evidence {evidence_key: $evidence_key}) SET e.id = $id, e.evidence_key = $evidence_key, e.version = $version, e.data = $data, e.updated_at = coalesce($updated_at, datetime()), e.last_checked = $last_checked';
 ...
 */
async function processTagNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  let maxTimestamp: Date | undefined;
...
async function processArtifactNodesTest(session: Session | null, supabase: SupabaseClient, since?: string, graphSparse?: boolean): Promise<ProcessResult> {
  let created = 0;
  let processed = 0;
  let maxTimestamp: Date | undefined;
    ...
    for (const artifact of artifacts) {
      ...
      if (ledger.supports && Array.isArray(ledger.supports)) {
        for (const targetId of ledger.supports) {
          const cypher = 'MATCH (a:Artifact {id: $artifactId}) MATCH (c:Concept {id: $targetId}) MERGE (a)-[:SUPPORTS]->(c)';
      ...
      if (ledger.contrasts && Array.isArray(ledger.contrasts)) {
        for (const targetId of ledger.contrasts) {
          const cypher = 'MATCH (a:Artifact {id: $artifactId}) MATCH (c:Concept {id: $targetId}) MERGE (a)-[:CONTRASTS]->(c)';
