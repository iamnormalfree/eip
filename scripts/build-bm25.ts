...
// Data Models from Synthesis Blueprint
export interface IndexBuildResult {
  ...
  schema_registry_entry: SchemaRegistryEntry;
  error?: string;
}
...
class BM25IndexBuilder {
   ...
   */
  async buildIndex(sources: DataSource[], parentVersion?: string): Promise<IndexBuildResult> {
    ...
    try {
      ...
      const schemaEntry: SchemaRegistryEntry = {
      ...
      return {
        ...
        document_count: 0,
        build_time_ms: buildTime,
        schema_registry_entry: {} as SchemaRegistryEntry,
        error: errorMessage
      };
    }
  }
   ...
   */
  async loadIndex(indexPath: string): Promise<BM25IndexData> {
    try {
   ...
   */
  validateChecksum(indexPath: string, expectedChecksum: string): boolean {
    try {
      if (!fs.existsSync(indexPath)) {
   ...
   */
  async fetchSourcesFromSupabase(limit: number = 1000): Promise<DataSource[]> {
    try {
   ...
   */
  private processDocuments(sources: DataSource[]): Array<{
    id: string;
    fields: {
      concept_abstract?: string;
      artifact_summary?: string;
   ...
   */
  private combineEntityContent(entity: any): string {
   ...
   */
  private calculateAverageDocLength(documents: Array<{
    field_terms: {
      concept_abstract?: string[];
      artifact_summary?: string[];
      entity_name?: string[];
   ...
   * Create index directory structure
   */
  private async createIndexDirectory(version: string): Promise<string> {
   ...
   */
  private async writeIndexAtomically(filePath: string, indexData: BM25IndexData): Promise<void> {
   ...
   */
  private async registerIndexInSchema(entry: SchemaRegistryEntry): Promise<void> {
    try {
