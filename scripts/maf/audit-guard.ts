...
async function runAudit(beadId: string, sampleSize: number, agentId: string, auditId: string, executionId: string): Promise<MafAuditGuardResult> {
  ...
  try {
    ...
    let status: 'passed' | 'failed' | 'warnings';
    ...
    } as unknown as MafAuditGuardResult;
 ...
 */
function formatOutput(result: any, json: boolean = false): void {
  if (json) {
    console.log(JSON.stringify(result, null, 2));
  } else {
    const statusIcon = result.status === 'passed' ? '✅' : result.status === 'warnings' ? '⚠️' : '❌';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await runtime.enqueue(auditRequest as any);
