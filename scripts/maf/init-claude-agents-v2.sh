...
init_claude_agent() {
    ...
    tmux send-keys -t "$SESSION_NAME:0.$pane" "claude --settings ${PROJECT_ROOT}/.claude" Enter
    sleep 3
...
sleep 1
tmux send-keys -t "$SESSION_NAME:0.4" "claude --settings ${PROJECT_ROOT}/.claude" Enter
sleep 3
...
tmux send-keys -t "$SESSION_NAME:0.0" "echo 'Settings: ${PROJECT_ROOT}/.claude'" Enter
