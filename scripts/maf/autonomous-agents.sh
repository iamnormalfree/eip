...
while true; do
    ...
    if python3 ${PROJECT_ROOT}/scripts/maf/bead-assigner.py status | grep -q "$bead_id"; then
...
tmux send-keys -t $SESSION_NAME:0.0 "echo ''" Enter
tmux send-keys -t $SESSION_NAME:0.0 "watch -n 60 'echo \"Last update: \$(date)\" && python3 ${PROJECT_ROOT}/scripts/maf/bead-assigner.py status | grep -E \"Ready|Reserved\"'" Enter
