...
import { SupervisorDecisionEngine, createSupervisorDecisionEngine, SupervisorActionType } from '../../lib/maf/supervision/decision-engine';
...
class CliCommandExecutor {
  ...
  async executeSupervisorAction(
    ...
    switch (action) {
      ...
      case SupervisorActionType.PEER_REVIEW:
      case SupervisorActionType.CONTINUE:
        return {
          success: true,
          result: {
            action: 'logged_only',
      ...
      case 'RESTART' as SupervisorActionType:
        if (!agentId) {
      ...
      case 'LOG_AND_CONTINUE' as SupervisorActionType:
        return {
          success: true,
          result: {
            action: 'logged_only',
      ...
      case 'PAUSE' as SupervisorActionType:
      case 'ESCALATE' as SupervisorActionType:
      case 'QUARANTINE' as SupervisorActionType:
      case 'RESOURCE_BOOST' as SupervisorActionType:
      case 'WORKLOAD_REDUCTION' as SupervisorActionType:
      case 'REASSIGN_TASK' as SupervisorActionType:
      case 'EMERGENCY_STOP' as SupervisorActionType:
        return {
          success: true,
          result: {
            action: 'logged_only',
...
class SupervisorCLI {
  ...
  constructor(config: Partial<SupervisorCliConfig> = {}) {
    ...
    this.decisionEngine = createSupervisorDecisionEngine(
      this.dataCollector as any,
      decisionTable,
      thresholdManager as any
    ) as unknown as SupervisorDecisionEngine;
  ...
  async runSupervisionCycle(): Promise<{
    ...
    try {
      ...
      const decisionResult = await (this.decisionEngine as any).evaluateAllAgents(supervisionContext);
  ...
  private async executeSupervisorActions(
    ...
    for (const [agentId, decision] of Object.entries(decisions)) {
      try {
        ...
        if (!decision || !(decision as any).recommendedAction) {
        ...
        const action = (decision as any).recommendedAction;
        let success = false;
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        const action = (decision as any)?.recommendedAction || 'LOG_AND_CONTINUE';
    ...
    const actionTypes = results.reduce((acc, result) => {
      acc[result.action as unknown as string] = (acc[result.action as unknown as string] || 0) + 1;
      return acc;
  ...
  private async logCycleMetrics(metrics: {
    ...
    try {
      this.eventLogger.logPerformanceThreshold({
        threshold_type: 'supervisor_cycle' as any,
  ...
  private calculateSystemHealth(loadLevel: LoadLevel, totalAgents: number): SystemHealthStatus {
    ...
    if (loadLevel === LoadLevel.CRITICAL) return SystemHealthStatus.CRITICAL;
