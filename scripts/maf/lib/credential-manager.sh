...
# NOTE: PROJECT_ROOT should be set by parent script (spawn-agents.sh or prompt-agent.sh)
# We only set it here if not already set
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Detect subtree layout and adjust PROJECT_ROOT accordingly
# Only set PROJECT_ROOT if not already set by parent script
