...
if [ -f "$PROJECT_ROOT/maf/lib/maf/preflight-coordinator.ts" ]; then
    MAF_SOURCE_ROOT="$PROJECT_ROOT/maf"
else
    MAF_SOURCE_ROOT="$PROJECT_ROOT"
fi
...
validate_environment() {
    ...
    local coordinator_file="${MAF_SOURCE_ROOT}/lib/maf/preflight-coordinator.ts"
...
setup_environment() {
    ...
    if [ -f "${PROJECT_ROOT}/.env.local" ]; then
        ...
        set -a
        source <(grep -v '^#' "${PROJECT_ROOT}/.env.local" | grep -v '^$' | sed 's/^export//' | sed 's/=/="/;s/$/"/')
        set +a
...
run_preflight_check() {
    ...
    if [ -f "dist/lib/maf/preflight-coordinator.js" ]; then
        ...
        if [ "$OUTPUT_FORMAT" = "json" ]; then
            export NODE_PATH="${PROJECT_ROOT}/dist:${NODE_PATH:-}"
