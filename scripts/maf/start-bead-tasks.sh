...
python3 ${PROJECT_ROOT}/scripts/maf/bead-assigner.py status 2>/dev/null | grep -A 10 "Reserved:" || echo "Could not fetch assignments"
...
tmux send-keys -t "$SESSION_NAME:0.4" "python3 ${PROJECT_ROOT}/scripts/maf/bead-assigner.py status | grep -A 5 'Reserved:'" Enter
